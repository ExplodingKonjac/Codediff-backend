...
class Session(db.Model):
    ...
    gen_code = db.Column(db.JSON, default=lambda: {'lang': 'cpp', 'std': 'c++17', 'content': ''})
...
class TestCase(db.Model):
    ...
    status = db.Column(db.String(10), default='PENDING')  # AC, WA, TLE, MLE, RE
    input_data = db.Column(db.Text, nullable=False)
    user_output = db.Column(db.Text)
    std_output = db.Column(db.Text)
    time_cost = db.Column(db.Float, default=0.0)  # ms
    memory_cost = db.Column(db.Float, default=0.0)  # MB
    created_at = db.Column(db.DateTime, default=_now_fn)
