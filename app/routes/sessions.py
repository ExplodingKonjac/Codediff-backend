...
from app.schemas.session import SessionCreateSchema, SessionUpdateSchema, SessionListQuerySchema
    ...
    def post(self):
        ...
        session = Session(
            ...
            user_code={
                ...
                'content': data['user_code'].get('content', '')
            },
            std_code={
                'lang': data['std_code'].get('lang', 'cpp'),
                'std': data['std_code'].get('std', 'c++17'),
                'content': data['std_code'].get('content', '')
            }
        )
        
        # 可选字段
        if 'gen_code' in data and data['gen_code'] and data['gen_code'].get('content'):
             session.gen_code = {
                'lang': data['gen_code'].get('lang', 'cpp'),
                'std': data['gen_code'].get('std', 'c++17'),
                'content': data['gen_code'].get('content', '')
            }
    ...
    def put(self, session_id):
        ...
        for field in code_fields:
            if field in data and isinstance(data[field], dict):
                ...
                # Ensure we handle nested schema update correctly
                updated_value = {
